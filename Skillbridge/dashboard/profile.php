    ...
    if (isset($_POST['update_profile'])) {
        try {
            ...
            if ($role === 'Student') {
                ...
                $github = trim($_POST['github'] ?? ''); // ✅ NEW: GitHub field
                $linkedin = trim($_POST['linkedin'] ?? ''); // ✅ NEW: LinkedIn field
                ...
                if ($exists) {
    // ✅ FIXED: Removed extra comma, added github and linkedin
    $stmt = $conn->prepare("UPDATE student_profiles SET university=?, course=?, graduation_year=?, cgpa=?, skills=?, bio=?, year_of_study=?, github=?, linkedin=?, resume_path=? WHERE user_id=?");
    $stmt->bind_param("ssidssisssi", $university, $course, $graduation_year, $cgpa, $skills, $bio, $year_of_study, $github, $linkedin, $resume_path, $user_id);
} else {
    // ✅ FIXED: Added github and linkedin values (11 parameters total)
    $stmt = $conn->prepare("INSERT INTO student_profiles (user_id, university, course, graduation_year, cgpa, skills, bio, year_of_study, github, linkedin, resume_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("issidssisss", $user_id, $university, $course, $graduation_year, $cgpa, $skills, $bio, $year_of_study, $github, $linkedin, $resume_path);
}
...
<div class="nav-section">
    ...
    <a href="<?php 
        if ($role === 'Admin') {
            ...
            echo BASE_URL . '/dashboard/' . strtolower($role) . '.php';
        }
    ?>" class="nav-item">
                            ...
                            <div class="form-grid">
                                ...
                                <div class="form-group form-group-full">
                                            <label for="github" class="form-label">
                                                <i class="fab fa-github"></i> GitHub Profile URL
                                            </label>
                                            <input type="url" id="github" name="github" class="form-control" 
                                                value="<?php echo htmlspecialchars($profile_data['github'] ?? ''); ?>" 
                                                placeholder="https://github.com/yourusername">
                                            <small style="color: #64748b; font-size: 0.85rem; margin-top: 0.25rem; display: block;">
                                                <i class="fas fa-info-circle"></i> Share your GitHub profile to showcase your projects and code
                                            </small>
                                        </div>
                                        ...
                                        <div class="form-group form-group-full">
                                            <label for="linkedin" class="form-label">
                                                <i class="fab fa-linkedin"></i> LinkedIn Profile URL
                                            </label>
                                            <input type="url" id="linkedin" name="linkedin" class="form-control" 
                                                value="<?php echo htmlspecialchars($profile_data['linkedin'] ?? ''); ?>" 
                                                placeholder="https://linkedin.com/in/yourprofile">
                                            <small style="color: #64748b; font-size: 0.85rem; margin-top: 0.25rem; display: block;">
                                                <i class="fas fa-info-circle"></i> Connect your professional network and career profile
                                            </small>
                                        </div>
                                ...
                                <label for="resume" class="form-label">Resume (PDF only)</label>
                                        ...
                                        <?php if (!empty($profile_data['resume_path'])): ?>
                                            <div class="current-resume">
                                                ...
                                                <!-- ✅ NEW: Download Button -->
                                                <a href="<?php echo BASE_URL . '/' . $profile_data['resume_path']; ?>" download style="margin-left: 0.5rem; color: #3b82f6;">
