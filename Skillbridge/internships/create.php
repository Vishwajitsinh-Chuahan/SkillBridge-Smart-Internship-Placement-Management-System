...
// ✅ Get company data with proper NULL handling
...
if (!$company_data || $company_data['status'] !== 'approved') {
    $_SESSION['error'] = 'Your company must be approved before posting internships.';
    header('Location: ../dashboard/company.php');
    exit();
}

// ✅ Properly handle NULL values and ensure integer type
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_custom_skill'])) {
    $custom_skill = trim($_POST['custom_skill_name']);
    ...
    if (!empty($custom_skill)) {
        ...
        if ($check_result->num_rows == 0) {
            ...
            if ($insert_stmt->execute()) {
                $success_message = "Skill '$custom_skill' added successfully!";
            }
        } else {
            $error_message = "Skill '$custom_skill' already exists.";
        }
    }
}

// ✅ NEW: Handle form submission with start_date/end_date automation
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['post_internship'])) {
    ...
    // ✅ NEW: Get start_date and end_date
    $start_date = $_POST['start_date'] ?? date('Y-m-d'); // Default: today
    $end_date = $_POST['end_date'] ?? $application_deadline; // Default: same as deadline
    ...
    if (empty($title) || empty($description) || empty($requirements) || empty($location) || empty($application_deadline)) {
    ...
    } elseif (strtotime($end_date) > strtotime($application_deadline)) {
        $error_message = 'End date cannot be after application deadline.';
    } else {
        // ✅ NEW AUTOMATION LOGIC with start_date/end_date
        $needs_approval = false;
        $today = date('Y-m-d');
        
        if ($current_posts_count >= 5) {
            // ✅ Trusted company (6+ approved posts)
            
            if ($start_date <= $today) {
                // Start date is today or past → Make ACTIVE immediately
                $internship_status = 'active';
                $success_msg = '🎉 Internship is now LIVE! (Auto-approved - trusted company)';
            } else {
                // Start date is future → Set as APPROVED, will auto-activate later
                $internship_status = 'approved';
                $success_msg = "Internship scheduled! Will automatically go live on " . date('M d, Y', strtotime($start_date)) . ".";
            }
            
            $needs_approval = false; // No admin approval needed
            
        } else {
            // ✅ New company (1-5 posts) - needs admin approval first
            $internship_status = 'pending';
            $needs_approval = true;
            $success_msg = "Internship posted! Waiting for admin approval. Will go live on " . date('M d, Y', strtotime($start_date)) . " after approval.";
        }
        
        // ✅ Insert internship with start_date and end_date
        $stmt = $conn->prepare("INSERT INTO internships (company_id, title, description, requirements, skills_required, internship_type, location, location_type, duration, stipend, positions_available, application_deadline, start_date, end_date, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt->execute()) {
            // ✅ If auto-approved, increment count
            if (!$needs_approval) {
                ...
                if ($new_count >= 10 && $current_trust_level === 'verified') {
                    ...
                    $_SESSION['success'] = '🎉 Internship posted successfully! You have been upgraded to TRUSTED status!';
                } else {
...
<head>
    ...
    <title>Post Internship - SkillBridge</title>
    ...
    <style>
        * {
        ...
        /* Sidebar Styling - Matching company.php */
        .sidebar {
            position: fixed;
            left: 0;
            top: 0;
    ...
    <div class="main-content">
        <div class="top-bar">
            <div class="page-title">
                ...
                <p>Create an internship opportunity and attract talented students</p>
            </div>
        </div>
            ...
            <div class="form-container">
                <form method="POST" action="">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" class="form-control" placeholder="e.g., Full Stack Developer Intern" required maxlength="200">
                    </div>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="positions_available" class="form-control" min="1" value="1" required>
                        </div>
                    </div>

                    <div class="form-group">
                        <label>Description <span class="required">*</span></label>
                        <textarea name="description" class="form-control" placeholder="Provide a detailed description of the role, responsibilities, and what the candidate will learn..." required maxlength="5000"></textarea>
                        <div class="character-count">Maximum 5000 characters</div>
                    </div>

                    <div class="form-group">
                        <label>Requirements & Qualifications <span class="required">*</span></label>
                        <textarea name="requirements" class="form-control" placeholder="List the required qualifications, education, experience, and any specific prerequisites..." required maxlength="3000"></textarea>
                    ...
                    <div class="skills-section">
                        <label style="font-weight: 600; margin-bottom: 1rem; display: block;">Select from Available Skills</label>
                        <div class="skills-checkbox-group">
                            <?php if (!empty($all_skills)): ?>
                                <?php foreach ($all_skills as $category => $skills): ?>
                                    ...
                                    <?php foreach ($skills as $skill): ?>
                                        <div class="skill-item">
                                            <input type="checkbox" name="skills_required[]" value="<?php echo htmlspecialchars($skill['skill_name']); ?>" id="skill_<?php echo $skill['id']; ?>">
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <p style="grid-column: 1 / -1; color: #6c757d; padding: 1rem;">No skills available in database.</p>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="custom-skills-box">
                            ...
                            <p class="help-text" style="margin-bottom: 1rem;">Can't find the skill you need? Add custom skills separated by commas (e.g., React Native, GraphQL, Docker)</p>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="location" class="form-control" placeholder="e.g., Mumbai, Maharashtra" required>
                        </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="duration" class="form-control" placeholder="e.g., 3 months, 6 months">
                        </div>

                        <div class="form-group">
                            <label>Stipend/Salary</label>
                            <input type="text" name="stipend" class="form-control" placeholder="e.g., ₹10,000/month, ₹5-7 LPA">
                        </div>
                    </div>

                    <div class="form-group">
                        <label>Application Deadline <span class="required">*</span></label>
                        <input type="date" name="application_deadline" class="form-control" required min="<?php echo date('Y-m-d', strtotime('+1 day')); ?>">
                    </div>
                    
                    <div class="form-row" style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="form-group">
                        <label for="start_date" class="form-label">
                            <i class="fas fa-calendar-check"></i> Start Date (Go Live Date) *
                        </label>
                        <input type="date" 
                            id="start_date" 
                            name="start_date" 
                            ...
                            required>
                        <small style="display: block; margin-top: 0.5rem; color: #64748b; font-size: 0.85rem;">
                            <i class="fas fa-info-circle"></i> 
                            <?php if ($current_posts_count >= 5): ?>
                                Internship will automatically go live on this date (no approval needed)
                            <?php else: ?>
                                Internship will go live on this date after admin approval
                            <?php endif; ?>
                        </small>
                    </div>
                    ...
                    <div class="form-group">
                        <label for="end_date" class="form-label">
                            <i class="fas fa-calendar-times"></i> End Date (Auto-Close Date) *
                        </label>
                        <input type="date" 
                            id="end_date" 
                            name="end_date" 
                            ...
                            required>
                        <small style="display: block; margin-top: 0.5rem; color: #64748b; font-size: 0.85rem;">
                            <i class="fas fa-info-circle"></i> Internship will automatically close on this date
                        </small>
                    </div>
                </div>
                    ...
                    <div class="form-buttons">
                        ...
                        <a href="../dashboard/company.php" class="btn-secondary">
     ...
     <script>
        document.getElementById('start_date').addEventListener('change', function() {
            ...
            document.getElementById('application_deadline').setAttribute('min', startDate);
        });

        document.getElementById('end_date').addEventListener('change', function() {
            const endDate = this.value;
            const deadlineInput = document.getElementById('application_deadline');
            if (deadlineInput.value && deadlineInput.value > endDate) {
                alert('Application deadline cannot be after end date');
                this.value = '';
            }
        });
    </script>
