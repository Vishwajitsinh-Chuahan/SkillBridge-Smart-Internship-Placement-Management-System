...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_internship'])) {
    ...
    // Validation
    if (empty($title) || empty($description) || empty($requirements) || empty($location) || empty($application_deadline) || empty($start_date) || empty($end_date)) {
        $error_message = 'Please fill in all required fields.';
    } else {
        // Update internship
        $stmt = $conn->prepare("UPDATE internships SET title = ?, description = ?, requirements = ?, skills_required = ?, internship_type = ?, location = ?, location_type = ?, duration = ?, stipend = ?, positions_available = ?, application_deadline = ?, start_date = ?,end_date = ?, updated_at = NOW() WHERE id = ? AND company_id = ?");
