...
if (isset($_POST['approve_internship'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        // 1. Update internship status to approved
        $update_internship = $conn->prepare("UPDATE internships SET status = 'approved', updated_at = NOW() WHERE id = ?");
        $update_internship->bind_param("i", $internship_id);
        ...
        if ($new_count == 5 && $current_trust_level === 'new') {
            ...
            $success_message = "✅ Internship approved! Company '{$internship['company_name']}' has been upgraded to VERIFIED status (5 approved posts). Future posts will be auto-approved!";
        ...
        elseif ($new_count == 10 && $current_trust_level === 'verified') {
            ...
            $success_message = "✅ Internship approved! Company '{$internship['company_name']}' has been upgraded to TRUSTED status (10 approved posts)!";
            
            // Log the upgrade
            error_log("Admin {$admin_name} approved internship ID: {$internship_id}. Company ID: {$company_id} upgraded to TRUSTED.");
        }
        else {
            $success_message = "✅ Internship approved successfully! ({$new_count}/5 posts for VERIFIED status)";
        }
        
        // Log admin action
        error_log("Admin {$admin_name} approved internship ID: {$internship_id} for company ID: {$company_id}");
    } else {
...
// ✅ FIXED: Handle Reject Action with rejection_reason saved to database
if (isset($_POST['reject_internship'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        // ✅ FIX: Update internship status AND save rejection_reason
