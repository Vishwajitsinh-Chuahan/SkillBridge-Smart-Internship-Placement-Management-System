...
// ✅ ENHANCED SEARCH: Search across multiple columns including company name and student resume
if (!empty($search)) {
    $where_conditions[] = "(u.full_name LIKE ? OR u.email LIKE ? OR u.username LIKE ? OR c.name LIKE ? OR sp.resume_path LIKE ?)";
    $search_param = "%$search%";
    $params[] = $search_param;
    $params[] = $search_param;
    $params[] = $search_param;
...
// ✅ UPDATED QUERY: Include student resume information
$users_query = "
    SELECT u.id, u.username, u.full_name, u.email, u.phone, u.status, u.created_at,
           ...
           CASE 
               WHEN r.role_name = 'Student' THEN sp.education
               ELSE NULL
           END as student_education
    FROM users u 
    JOIN roles r ON u.role_id = r.id 
    LEFT JOIN companies c ON u.id = c.user_id AND r.role_name = 'Company'
    LEFT JOIN student_profiles sp ON u.id = sp.student_id AND r.role_name = 'Student'
    $where_clause
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="filter-section">
            ...
            <div class="search-actions">
                <form class="search-box" method="GET">
                    ...
                    <input type="text" name="search" placeholder="Search users, email, company..." 
        ...
        <!-- ✅ UPDATED: Bulk Actions Bar - Only shown when checkboxes selected -->
                            ...
                            <?php if ($users_result && $users_result->num_rows > 0): ?>
                                <?php while ($user = $users_result->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <!-- ✅ NEW: Student Resume & Education Display -->
                                            <?php if ($user['role_name'] === 'Student'): ?>
                                                ...
                                                <?php else: ?>
                                                    <span style="color: #94a3b8; font-style: italic; font-size: 0.75rem;">No Resume</span>
                                                <?php endif; ?>
                                                
                                                <?php if ($user['student_skills'] || $user['student_education']): ?>
                                                    <div class="student-info">
                                                        <?php if ($user['student_skills']): ?>
                                                            <div><strong>Skills:</strong> <?php echo htmlspecialchars(substr($user['student_skills'], 0, 50)); ?><?php echo strlen($user['student_skills']) > 50 ? '...' : ''; ?></div>
                                                        <?php endif; ?>
                                                        <?php if ($user['student_education']): ?>
                                                            <div><strong>Education:</strong> <?php echo htmlspecialchars(substr($user['student_education'], 0, 50)); ?><?php echo strlen($user['student_education']) > 50 ? '...' : ''; ?></div>
                                                        <?php endif; ?>
                                                    </div>
                                                <?php endif; ?>
                                            <?php elseif ($user['role_name'] === 'Company' && $user['company_name']): ?>
                                                <div style="font-size: 0.875rem;">
                                                    <div style="font-weight: 600;"><?php echo htmlspecialchars($user['company_name']); ?></div>
                                                    <div style="color: #64748b; font-size: 0.75rem;">Company Profile</div>
                                                </div>
                                            <?php else: ?>
